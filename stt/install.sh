#!/usr/bin/env bash
#
# ECodeWhisper STT Service Installer
...
#   --cpu              Use CPU-only image (no NVIDIA GPU required)
#   --uninstall        Remove the service
#   --help             Show this help
#
# Requires: sudo, docker, docker-compose-plugin
# GPU mode also requires: NVIDIA GPU with drivers
...
check_requirements() {
    ...
    # Check NVIDIA GPU (only for GPU mode)
    if [[ "$USE_CPU" == false ]]; then
        if ! command -v nvidia-smi &>/dev/null; then
            log_warn "nvidia-smi not found - consider using --cpu flag"
        fi
    fi
...
create_compose_file_gpu() {
    cat > "${INSTALL_DIR}/compose.yml" << EOF
# ECodeWhisper WhisperLive STT Service (GPU)
...
create_compose_file_cpu() {
    cat > "${INSTALL_DIR}/compose.yml" << EOF
# ECodeWhisper WhisperLive STT Service (CPU)
# Auto-generated by install.sh
...
create_compose_file() {
    ...
    if [[ "$USE_CPU" == true ]]; then
        log_info "Using CPU-only image (no GPU required)"
        create_compose_file_cpu
    else
        log_info "Using GPU image (NVIDIA GPU required)"
        create_compose_file_gpu
    fi
...
[Unit]
Description=ECodeWhisper WhisperLive STT Service
...
show_status() {
    ...
    echo "  Language: ${WHISPER_LANGUAGE} (set in extension settings)"
    echo "  Port:     ${STT_PORT}"
    echo ""
    ...
    echo "  ecodewhisper.whisperEndpoint: ws://localhost:${STT_PORT}"
    echo "  ecodewhisper.model: ${WHISPER_MODEL}"
    echo "  ecodewhisper.language: ${WHISPER_LANGUAGE}"
    echo ""
}
